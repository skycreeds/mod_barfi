...
def load_schemas(user:str):
    ...
    not_user_schema_names = [x for x in schema_names if x.split('@')[1] != hashlib.sha256(user.encode()).hexdigest()]
    for i in not_user_schema_names:
        del schemas[i]
    user_schema=filter(lambda i: i not in not_user_schema_names, schema_names)
    return {'schema_names':user_schema , 'schemas': schemas}


def save_schema(schema_name: str, schema_data: Dict,user:str):
    try:
        with open('schemas.barfi', 'rb') as handle_read:
            schemas = pickle.load(handle_read)
    except FileNotFoundError:
